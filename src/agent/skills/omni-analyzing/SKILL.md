---
name: omni-analyzing
description: 制品一致性分析技能. 对 spec.md、design.md 和 tasks.md 执行非破坏性的跨制品一致性和质量分析. 在任务生成后使用, 用于识别不一致、重复、模糊和规范不足的项目. 当用户提到分析制品、检查一致性、质量审查、或在 omni.tasks 完成后需要验证时使用.
---
...
**章程权威**: 项目章程(`.specify/memory/constitution.md`)在此分析范围内是**不可协商的**. 章程冲突自动为严重问题, 需要调整规范、计划或任务——而不是稀释、重新解释或默默忽略原则. 如果原则本身需要更改, 那必须在分析之外的单独、明确的章程更新中进行.
...
- 提供明确的命令建议: 例如, "运行 /omni.specify 进行细化"、"运行 /omni.design 调整架构"、"手动编辑 tasks.md 为 'performance-metrics' 添加覆盖"
