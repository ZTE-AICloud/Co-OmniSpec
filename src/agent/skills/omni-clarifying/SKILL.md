---
name: omni-clarifying
description: 规范澄清技能. 通过提出最多 5 个高度针对性的澄清问题, 识别当前功能规范中未充分说明的领域, 并将答案编码回规范中. 当用户提到澄清规范、消除模糊性、补充规范细节、或在 omni.specify 完成后需要进一步细化时使用.
---
...
注意: 此澄清工作流应在 `/omni.design` 之前运行(并完成). 如果用户明确表示跳过澄清(例如, 探索性原型), 可以继续, 但必须警告下游返工风险增加.
...
- 如果 JSON 解析失败, 中止并指示用户重新运行 `/omni.specify` 或验证功能分支环境.
