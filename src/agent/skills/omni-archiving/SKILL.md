...
1. 针对每条需求变更，根据动作类型执行：
   - **INSERT**：
     - 基于 `.specify/metamodel/1.requirement-template.md` 生成 `DOC_DIR/specs/requirements/REQ-XXX-*.md` 新文档。
...
1. 针对每条场景变更，根据动作类型执行：
   - **INSERT**：
     - 基于 `.specify/metamodel/3.scenario-template.md` 创建新场景文档。
...
1. 对每条逻辑实体变更按动作类型处理：
   - **INSERT**：
     - 使用 `.specify/metamodel/6.entity-template.md` 创建实体文档。
   - **MODIFY**：
...
1. 对每条功能变更按动作类型处理：
   - **INSERT**：
     - 使用 `.specify/metamodel/5.function-template.md` 创建功能文档。
...
1. 对 `CHANGES_DIR/contracts/` 下每个接口变更文档：
   ...
   - 对于 **INSERT**：
     - 使用 `.specify/metamodel/7.interface-template.md` 创建接口文档。
