...
**简化版配置**（推荐）：`{REPO_ROOT}/user_input/interface-config.md`

**YAML配置**（高级，向后兼容）：`{REPO_ROOT}/user_input/interface-identification-rules.yaml`
...
# 系统安装时会自动生成模板文件
# 如果不存在，可以手动创建：
cp .specify/templates/interface-config-simple.md user_input/interface-config.md
```
