...
# omni.reverse - 从代码库中反构各种类型的要素，生成标准化的要素文档
#
# 第一阶段支持接口清单（interfaces）反构
#
# Usage: omni-reverse.sh [OPTIONS]
#
# 详细用法请参考 --help
...
使用示例：
  # 反构整个代码库的接口
  omni-reverse.sh --target interfaces --path .

  # 反构指定目录的接口
  omni-reverse.sh --target interfaces --path src/api/,src/services/

  # 仅反构 RESTful 和消息类接口
  omni-reverse.sh --target interfaces --path src/ --interface-types restful,message

  # 反构指定文件的接口
  omni-reverse.sh --target interfaces --files src/api/user.py

  # 交互式反构
  omni-reverse.sh --target interfaces --path src/ --interactive
  omni-reverse.sh --target interfaces --path src/ --interactive yes

  # 预览模式
  omni-reverse.sh --target interfaces --path src/ --preview

  # JSON 输出
  omni-reverse.sh --target interfaces --path src/ --json

  # 增量反构
  omni-reverse.sh --target interfaces --incremental --git-diff HEAD~1 --merge
    ...
    if [[ -n "${REVERSE_CACHE_DIR:-}" ]]; then
        ...
        if ! cache_dir=$(normalize_path "$REVERSE_CACHE_DIR" "$repo_root" 2>/dev/null); then
            ...
            if [[ -n "${REVERSE_CACHE_DIR:-}" ]]; then
                ...
                # 默认缓存路径构造：仓库根目录/.omni-reverse/cache/目标要素类型
                echo "${repo_root}/.omni-reverse/cache/${target}"
            fi
        fi
        else
            echo "$cache_dir"
        ...
        echo "${repo_root}/.cache/omni-reverse/${target}"
    fi
}
...
find_template() {
    ...
    # 2. 项目自定义模板
    local project_template="${repo_root}/.specify/templates/${template_filename}"
