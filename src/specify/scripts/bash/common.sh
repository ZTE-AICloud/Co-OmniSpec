...
get_project_name() {
    ...
    # 3. 从简单的文本文件读取
    local project_name_file="${repo_root}/.specify/project-name"
...
find_template_file() {
    ...
    if project_name=$(get_project_name "$repo_root" 2>/dev/null); then
        local project_template="${repo_root}/.specify/templates/${project_name}/${template_filename}"
    ...
    # 3. 项目根目录下的默认模板（向后兼容）
    local project_default_template="${repo_root}/.specify/templates/${template_filename}"
    ...
    local project_default_dir_template="${repo_root}/.specify/templates/default/${template_filename}"
