...
create_test_environment() {
    ...
    local cache_dir="$test_dir/.cache/omni-reverse/interfaces"
    local output_dir="$test_dir/omni-doc/interfaces"
    mkdir -p "$cache_dir"
    mkdir -p "$output_dir"
...
EOF
    ...
    # 创建模板文件
    cat > "$test_dir/.specify/templates/reverse-interface-detail-template.md" << 'EOF'
# 接口文档：{{interface_name}}
...
EOF
    ...
    cat > "$test_dir/.specify/templates/reverse-interface-inventory-template.md" << 'EOF'
# 接口清单
