...
"""
    ...
    def _get_batch_files(self) -> List[Path]:
        ...
        """
        # 读取批次索引文件
        cache_dir = self.repo_root / ".cache" / "omni-reverse" / "entities" / "entity-extraction"
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='输出目录（可选，默认使用 {REPO_ROOT}/.cache/omni-reverse/entities/entity-extraction/）'
    )
    parser.add_argument(
        '--max-workers',
        type=int,
    ...
    try:
        ...
        if args.interface_aggregation_dir:
            ...
            # 默认路径：从接口反构的缓存目录读取
            interface_aggregation_dir = repo_root / ".cache" / "omni-reverse" / "interfaces" / "interface-aggregation"
        ...
        if args.output_dir:
            ...
            # 默认路径：实体抽取缓存目录
            output_dir = repo_root / ".cache" / "omni-reverse" / "entities" / "entity-extraction"
