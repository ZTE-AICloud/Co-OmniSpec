...
使用方法:
    .\generate-interface-batches.ps1 -RepoRoot <repo_root> -FileList <file_list_json> [-BatchSize <size>] [-MaxTokens <tokens>]
...
function Main {
    ...
    foreach ($filePath in $files) {
        ...
        if (($currentBatchFiles.Count -ge $BatchSize) -or
            ...
            # 创建批次对象（文件使用对象格式，包含path和status）
            $fileEntries = $currentBatchFiles | ForEach-Object {
                @{
                    path = $_
                    status = "pending"
                }
