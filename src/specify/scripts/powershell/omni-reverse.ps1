...
#
# Usage: omni-reverse.ps1 [OPTIONS]
#
# 详细用法请参考 -Help
...
使用示例：
  # 反构整个代码库的接口
  .\omni-reverse.ps1 --target interfaces --path .

  # 反构指定目录的接口
  .\omni-reverse.ps1 --target interfaces --path src/api/,src/services/

  # 仅反构 RESTful 和消息类接口
  .\omni-reverse.ps1 --target interfaces --path src/ --interface-types restful,message

  # 反构指定文件的接口
  .\omni-reverse.ps1 --target interfaces --files src/api/user.py

  # 交互式反构
  .\omni-reverse.ps1 --target interfaces --path src/ --interactive

  # 预览模式
  .\omni-reverse.ps1 --target interfaces --path src/ --preview

  # JSON 输出
  .\omni-reverse.ps1 --target interfaces --path src/ --json

  # 增量反构
  .\omni-reverse.ps1 --target interfaces --incremental --git-diff HEAD~1 --merge
...
function Main {
    ...
    Write-LogInfo "开始 omni.reverse 命令执行..."
