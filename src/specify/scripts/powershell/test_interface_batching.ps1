...
function Create-TestEnvironment {
    ...
    $outputDir = Join-Path -Path $testDir -ChildPath "omni-doc/interfaces"
...
{{/description}}
"@
    $detailTemplate | Set-Content -Path (Join-Path -Path $testDir -ChildPath ".specify/templates/reverse-interface-detail-template.md") -Encoding UTF8
...
{{/interfaces}}
"@
    $inventoryTemplate | Set-Content -Path (Join-Path -Path $testDir -ChildPath ".specify/templates/reverse-interface-inventory-template.md") -Encoding UTF8
