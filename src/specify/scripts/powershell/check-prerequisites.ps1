...
if (-not (Test-Path $paths.FEATURE_DIR -PathType Container)) {
    ...
    Write-Output "Run /omni.specify first to create the feature structure."
    exit 1
}

if (-not (Test-Path $paths.IMPL_DESIGN -PathType Leaf)) {
    Write-Output "ERROR: design.md not found in $($paths.FEATURE_DIR)"
    Write-Output "Run /omni.design first to create the implementation design."
    exit 1
}
...
if ($RequireTasks -and -not (Test-Path $paths.TASKS -PathType Leaf)) {
    ...
    Write-Output "Run /omni.tasks first to create the task list."
    exit 1
}
