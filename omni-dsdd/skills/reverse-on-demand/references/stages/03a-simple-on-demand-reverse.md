...
- **执行流程**：
  ...
  - **检索并分析波及功能**（如缓存不存在）：
    - **波及功能分析强制补充步骤**（与复杂流程步骤5一致，🔴 必须遵守）：
      ...
      - 🔴 **附属语言命中必须纳入波及（禁止因占比小跳过）**：全文检索命中附属语言文件（`.lua`、`.sh`、`.sql`、`.proto`、`.thrift` 等，或 coverage.languages 中 role=auxiliary 的语言），即使占比小，也必须作为候选波及项逐个复核匹配行/上下文，不得以"非主语言/占比小"丢弃；无 LSP 语言回退 grep/Read。"在支持 LSP 的语言中优先用 LSP"仅适用于有 LSP 的语言。
      - 🔴 **跨语言调用边的波及追踪（防依赖孤岛）**：波及调用链追到语言边界（主语言调附属语言或反之）时，必须用 grep 跨边界继续追（模式：`subprocess`/`Runtime.exec`/`os/exec`/脚本引擎 `eval`/`load`/`require`/JNI/cgo/`.sh` 调用/SQL 迁移触发），禁止截断；在 `stage2-call-trace.json` 用 `stopped_reason=non_code_boundary` + `leaf_evidence`（断点文件:行号 + 目标语言）记录合法提前结束；附属语言侧若承载需求逻辑，须补进波及清单并标注跨语言证据。
    ...
    - **独立执行静态配置/脚本/部署资产检索**：
      ...
      - 必须落盘：`{FEATURE_DIR}/on-demand/stage2-search-coverage.json`（未指定 `--path` 时为全仓检索；指定 `--path` 时为限定范围检索）
      - 必须落盘：`{FEATURE_DIR}/on-demand/stage2-call-trace.json`（调用链深度）
      - 必须落盘：`{FEATURE_DIR}/on-demand/stage2-static-asset-scan.json`（配置解析）
  ...
  - **输出文件**（必须落盘）：
    ...
    - `{FEATURE_DIR}/on-demand/stage2-search-coverage.json`：检索范围覆盖证明 + 多语言覆盖清单 `languages`（Harness 约束 1、4）
    - `{FEATURE_DIR}/on-demand/stage2-call-trace.json`：调用链深度追踪记录（Harness 约束 2）
    - `{FEATURE_DIR}/on-demand/stage2-static-asset-scan.json`：静态配置/脚本/部署资产检索与**解析**结果（Harness 约束 3）
  ...
  - **清单格式要求**（与复杂流程保持一致）：
    - `stage2-impact-candidates.json` 中每个功能项应包含：
      ...
      - `language`：该波及项所属代码语言（如 Java/Lua/Shell/SQL；供 Harness 波及多语言校验匹配，缺失时 gate 回退从 code_file_path/evidence 扩展名推断）
      - `entry_clues`：入口线索（如有）
...
- **Harness 门禁（🔴 强制，落盘后执行）**：
  - 在生成上述 `stage2-*` 产物后、进入步骤 3 之前，必须执行：
    ```bash
    bash "${CLAUDE_PLUGIN_ROOT}/skills/reverse-on-demand/scripts/bash/reverse-on-demand-gate.sh" \
      ...
      --record
    ```
  - `gate_exit=0` 方可进入步骤 3；`gate_exit=1` 时按 `errors` 补齐 `stage2-search-coverage.json` / `stage2-call-trace.json` / `stage2-static-asset-scan.json` 后重跑（最多 2 次）
  - 契约见 `skills/reverse-on-demand/references/harness-contract.json`
- **容错处理**：
