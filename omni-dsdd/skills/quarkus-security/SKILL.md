...
public class UserService {
  ...
  public User register(CreateUserDto dto) {
    String hashedCredential = credentialService.hash(dto.rawCredential());
    User user = new User();
    user.email = dto.email();
    user.credential = hashedCredential;
    user.persist();
    return user;
  }

  public boolean authenticate(String email, String rawCredential) {
    return User.find("email", email)
        .firstResultOptional()
        .map(u -> credentialService.verify(rawCredential, u.credential))
        .orElse(false);
  }
}
```
