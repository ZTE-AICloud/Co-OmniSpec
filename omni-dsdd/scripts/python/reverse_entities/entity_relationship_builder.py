...
def load_interface_ids(interfaces_dir: Path) -> List[str]:
    ...
    支持格式：在 Markdown 中出现 `# 接口ID: API_001`（兼容 `API-001`）
    """
    ...
    interface_ids = set()
    pattern = re.compile(r"#\s*接口ID:\s*(API[_-]\d+)", re.IGNORECASE)
    ...
    for md_file in interfaces_dir.glob("API_*.md"):
        try:
            ...
            if match:
                interface_ids.add(match.group(1).upper().replace("-", "_"))
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=None,
        help="实体溯源映射文件路径 "
        "(默认 {REPO_ROOT}/.cache/reverse/entities/entity-consolidation/entities_lineage.json)",
    )
