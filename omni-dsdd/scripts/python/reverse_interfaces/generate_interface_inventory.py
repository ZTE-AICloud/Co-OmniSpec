...
def main():
    ...
    if not project_name:
        # 尝试从配置文件读取
        config_file = os.path.join(repo_root, '.omni-infra', 'config.yaml')
    ...
    if project_name:
        project_template = os.path.join(repo_root, '.omni-infra', 'templates', project_name, 'reverse-interface-inventory-template.md')
        if os.path.exists(project_template):
            template_file = project_template
    
    # 2. 尝试项目根目录下的默认模板（向后兼容）
    if not template_file:
        project_default = os.path.join(repo_root, '.omni-infra', 'templates', 'reverse-interface-inventory-template.md')
        if os.path.exists(project_default):
            template_file = project_default
    
    # 3. 尝试项目根目录下的 default 子目录模板
    if not template_file:
        project_default_dir = os.path.join(repo_root, '.omni-infra', 'templates', 'default', 'reverse-interface-inventory-template.md')
    ...
    if not template_file:
        ...
        system_default = os.path.join(omnispec_root, 'omni-infra', 'templates', 'default', 'reverse-interface-inventory-template.md')
        if os.path.exists(system_default):
            template_file = system_default
        else:
            # 向后兼容：系统根目录下的模板
            system_template = os.path.join(omnispec_root, 'omni-infra', 'templates', 'reverse-interface-inventory-template.md')
    ...
    # 默认输出目录：{REPO_ROOT}/omni-doc/specs/interfaces
    output_dir = os.path.join(repo_root, 'omni-doc', 'specs', 'interfaces')
    inventory_file = os.path.join(output_dir, '接口清单.md')
    interface_list_file = os.path.join(repo_root, '.cache', 'reverse', 'interfaces', 'interface-list.json')
    ...
    validator_script = os.path.join(
        ...
        'references',
        'scripts',
        'validate_and_fix_interface_doc_filenames.py'
    )
    if os.path.exists(validator_script):
        proc = subprocess.run(
            [sys.executable, validator_script, repo_root],
        ...
        if proc.returncode != 0:
            ...
            print("错误: 接口文档质量门禁未通过，禁止生成接口清单.md", file=sys.stderr)
            return 1
    else:
        log_error(f"质量门禁脚本未找到: {validator_script}")
        return 1
