...
find_repo_root() {
    local dir="$1"
    while [ "$dir" != "/" ]; do
        if [ -d "$dir/.git" ] || [ -d "$dir/.omni-infra" ]; then
            echo "$dir"
            return 0
        fi
        dir="$(dirname "$dir")"
...
if [[ -n "${CLAUDE_WORKING_DIR:-}" && -d "${CLAUDE_WORKING_DIR}" ]]; then
    REPO_ROOT="$(CDPATH="" cd "${CLAUDE_WORKING_DIR}" && pwd)"
    HAS_GIT=false  # Will probe Git later if needed
# Priority 2: Fall back to pwd (current working directory)
else
    REPO_ROOT="$(pwd)"
    HAS_GIT=false
fi
...
FEATURE_DIR="$CHANGES_DIR/$BRANCH_NAME"
mkdir -p "$FEATURE_DIR"

TEMPLATE="$REPO_ROOT/.omni-infra/templates/mini-design-template.md"
