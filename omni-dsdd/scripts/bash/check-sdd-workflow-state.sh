...
if [[ -n "${CLAUDE_WORKING_DIR:-}" && -d "${CLAUDE_WORKING_DIR}" ]]; then
  WORKING_DIR="$(CDPATH="" cd "${CLAUDE_WORKING_DIR}" && pwd)"
else
  WORKING_DIR="$(CDPATH="" cd "$SCRIPT_DIR/../../.." && pwd)"
fi
CHANGE_DIR="${WORKING_DIR}/changes"

# 查找 changes/<feature>/.runs/.omnispec-state.json（路径须含 RDC_ID）
state_path=$(find "${CHANGE_DIR}" -type f -name ".omnispec-state.json" -path "*/.runs/.omnispec-state.json" -path "*${RDC_ID}*" 2>/dev/null | head -1)
