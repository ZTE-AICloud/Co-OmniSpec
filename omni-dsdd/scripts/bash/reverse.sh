...
get_cache_dir() {
    local repo_root="$1"
    local target="$2"
    local default_cache_dir="${repo_root}/.cache/reverse/${target}"

    # 默认缓存路径构造：仓库根目录/.cache/reverse/目标要素类型

    # 支持通过环境变量指定缓存目录
    if [[ -z "${REVERSE_CACHE_DIR:-}" ]]; then
    ...
    if cache_dir=$(normalize_path "$REVERSE_CACHE_DIR" "$repo_root" 2>/dev/null); then
        ...
        log_warn "无法规范化环境变量 REVERSE_CACHE_DIR: $REVERSE_CACHE_DIR，使用默认路径: $default_cache_dir"
        echo "$default_cache_dir"
    fi
}
...
find_template() {
    ...
    # 2. 项目自定义模板
    local project_template="${repo_root}/.omni-infra/templates/${template_filename}"
...
main() {
    ...
    if [[ "$is_ai_mode" == "true" ]]; then
        ...
        log_info "功能开发进度请参考任务清单：omni-infra/design/omni_reverse_tasks.md"
        log_info ""
