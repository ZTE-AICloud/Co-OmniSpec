...
function Get-PluginRoot {
    if ($env:CLAUDE_PLUGIN_ROOT -and (Test-Path -LiteralPath $env:CLAUDE_PLUGIN_ROOT)) {
...
function Get-WorkingDir {
    if ($env:CLAUDE_WORKING_DIR -and (Test-Path -LiteralPath $env:CLAUDE_WORKING_DIR)) {
        return (Resolve-Path -LiteralPath $env:CLAUDE_WORKING_DIR).Path
    }
...
function Test-HasGit {
    ...
    if (-not $WorkingDir) { $WorkingDir = Get-WorkingDir }
    try {
        Push-Location $WorkingDir
...
function Get-FeaturePathsEnv {
    ...
    $hasGit = Test-HasGit -WorkingDir $workingDir

    $featureDir = $null
    if ($env:FEATURE_DIR -and (Test-Path -LiteralPath $env:FEATURE_DIR)) {
        $featureDir = (Resolve-Path -LiteralPath $env:FEATURE_DIR).Path
    } elseif ($env:OMNISPEC_FEATURE_DIR -and (Test-Path -LiteralPath $env:OMNISPEC_FEATURE_DIR)) {
        $featureDir = (Resolve-Path -LiteralPath $env:OMNISPEC_FEATURE_DIR).Path
    } else {
        $featureDir = Get-FeatureDir -RepoRoot $workingDir -Branch $currentBranch
    }

    $docDir = "omni-doc"
    $infraConfig = Join-Path $workingDir ".omni-infra/config"
    if (Test-Path -LiteralPath $infraConfig) {
        $line = Get-Content -LiteralPath $infraConfig | Where-Object { $_ -match '^DOC_DIR=' } | Select-Object -First 1
        if ($line) {
            $docDir = ($line -replace '^DOC_DIR=', '').Trim().Trim('"').Trim("'")
        }
    }
    ...
    [PSCustomObject]@{
        ...
        DOC_DIR = $docDir
        DOC_SPECS_DIR = Join-Path $workingDir "$docDir/specs"
        DOC_RULES_DIR = Join-Path $workingDir "$docDir/rules"
        DOC_NAVIGATIONS_DIR = Join-Path $workingDir "$docDir/navigations"
    }
}
...
function Get-ProjectName {
    ...
    $configFile = Join-Path $RepoRoot ".omni-infra/config.yaml"
    ...
    $projectNameFile = Join-Path $RepoRoot ".omni-infra/project-name"
...
function Find-TemplateFile {
    ...
    if ($projectName) {
        $projectTemplate = Join-Path $RepoRoot (".omni-infra/templates/$projectName/$TemplateFilename")
    ...
    $defaultTemplate = Join-Path $RepoRoot (".omni-infra/templates/$TemplateFilename")
    $searchedPaths += $defaultTemplate
    if (Test-Path -Path $defaultTemplate -PathType Leaf) {
        return $defaultTemplate
    }

    $defaultDirTemplate = Join-Path $RepoRoot (".omni-infra/templates/default/$TemplateFilename")
    ...
    if (-not [string]::IsNullOrEmpty($OmniSpecRoot)) {
        $systemDefaultDirTemplate = Join-Path $OmniSpecRoot ("omni-infra/templates/default/$TemplateFilename")
        ...
        $systemDefaultTemplate = Join-Path $OmniSpecRoot ("omni-infra/templates/$TemplateFilename")
