...
function Find-RepoRoot {
    ...
    while ($currentDir -ne "/") {
        ...
        if (Test-Path (Join-Path $currentDir ".omni-infra") -PathType Container) {
            return $currentDir
        }
...
# Priority 1: Use CLAUDE_WORKING_DIR if available (injected by runtime)
$ClaudeWorkingDir = $env:CLAUDE_WORKING_DIR
if (-not [string]::IsNullOrWhiteSpace($ClaudeWorkingDir) -and (Test-Path $ClaudeWorkingDir -PathType Container)) {
    $RepoRoot = (Resolve-Path $ClaudeWorkingDir).Path
    $HasGit = $false  # Will probe Git later if needed
}
# Priority 2: Fall back to current directory (pwd)
else {
    $RepoRoot = (Get-Location).Path
    $HasGit = $false
}

# Probe Git availability (only once, for branch operations)
...
$Template = Join-Path $RepoRoot ".omni-infra" "templates" "mini-design-template.md"
