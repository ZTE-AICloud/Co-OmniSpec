...
- **独立执行静态配置/脚本/部署资产检索**（如缓存不存在）：
  ...
  - 若需求关键词命中静态资产（如资源注解、环境变量、参数文件路径、模板变量），必须继续检查：
    ...
    - 生成/渲染/发布该文件的脚本
    - 消费这些配置的执行入口或运行时模块
  - 必须落盘：`{FEATURE_DIR}/on-demand/stage2-search-coverage.json`（全仓检索，Harness 约束 1）
  - 必须落盘：`{FEATURE_DIR}/on-demand/stage2-call-trace.json`（调用链深度，Harness 约束 2）
  - 必须落盘：`{FEATURE_DIR}/on-demand/stage2-static-asset-scan.json`（配置已解析，Harness 约束 3）
  - 落盘后必须执行 Harness：`reverse-on-demand-gate.sh --step stage2`（`gate_exit=0` 方可继续）
- **非显式调用链波及检索**（必须并行执行）：
  - **调度链检索**：
...
#### 5.1 加载模板文件
- **主文档模板**：读取 `{REPO_ROOT}/.omni-infra/templates/on-demand-function-impact-analysis-template.md` 模板文件（`REPO_ROOT` 为输入参数，若未提供则从 `FEATURE_DIR` 推导）
- **功能独立文档模板**：读取 `{REPO_ROOT}/.omni-infra/templates/on-demand-function-detail-template.md` 模板文件
