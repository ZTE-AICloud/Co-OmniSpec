...
基于阶段3的功能划分结果，为每一个功能**按照元模型模板 `.omni-infra/metamodel/5.function-template.md`** 生成功能详细文档文件。  
...
- **单功能文档文件名格式**（必须严格遵守，需与 `.omni-infra/metamodel/5.function-template.md` 保持一致）：
  - `FUNC-XXX-功能名称.md`
  - 示例：`FUNC-001-用户登录.md`
  - 要求：
...
- **单功能文档内部结构**：**必须完全符合 `.omni-infra/metamodel/5.function-template.md` 中定义的格式**：
...
- 本阶段在生成文档时，**应将 `.omni-infra/metamodel/5.function-template.md` 视为唯一权威模板**：  
...
- 当待处理功能数量 > 20 时：
  - 调用批次生成脚本按每批 5~10 个功能自动分批：
    - Linux/macOS（示例）：  
      `python3 ${CLAUDE_PLUGIN_ROOT}/skills/reverse-functions/scripts/python/generate_function_doc_batches.py --repo-root {REPO_ROOT} --function-doc-list function-doc-list.json`
  - 生成内容：
...
- 每轮最多启动 2 个子Agent（例如 `function-detail-writer` 子Agent）并行处理不同批次，或由主Agent直接处理：
  ...
  - 处理职责：
    - 对批次中的每个功能：
      ...
      - 先读取模板文件 `.omni-infra/metamodel/5.function-template.md` 的完整内容，基于模板原文替换占位符，禁止自行重写章节骨架
...
- 严格按照 `.omni-infra/metamodel/5.function-template.md` 填充Frontmatter字段与各章节内容（包括主流程与异常流程的PlantUML活动图骨架）
- 主Agent职责：
...
- **🔴 强制要求**：对每个功能，检查文档是否已正确生成到输出目录，且结构符合 `5.function-template.md`：
  ...
  - 正文中是否至少包含以下章节标题，且顺序不乱：
    ...
    - `## 性能要求（可选）`
    - `## 约束条件（可选）`
  - 主流程和异常流程中的PlantUML代码块是否以 ```plantuml 开头并以 `@enduml` 结尾，且基本结构与 `.omni-infra/metamodel/5.function-template.md` 中示例保持一致
- 对缺失或格式错误的文档：
...
- 文档文件名和内部结构必须严格按照 `.omni-infra/metamodel/5.function-template.md` 执行，这是后续工具和人工评审的**契约**，必须严格遵守
...
- 上一阶段：[03-function-partitioning.md](03-function-partitioning.md)