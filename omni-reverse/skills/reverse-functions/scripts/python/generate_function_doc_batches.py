...
"""
功能详情文档批次生成工具
基于输入列表（--function-doc-list）按"批次大小 + Token预算"动态分组，生成批次文件。
供 reverse-functions 对应阶段使用。

兼容 Windows 和 Linux 系统。
使用方法:
    python generate_function_doc_batches.py --repo-root <repo_root> --function-doc-list <list_json> [--batch-size <size>] [--max-tokens <tokens>]

参数:
    --repo-root:  仓库根目录路径
    --function-doc-list:     功能文档列表（JSON 格式：数组，或 {"items": [...]} / {"files": [...]}）
...
产出（写入 {repo_root}/.cache/reverse/functions/）:
    ...
    - function_doc_generation-batch-status.json
"""
import os
import sys
import json
...
def main():
    parser = argparse.ArgumentParser(description='功能详情文档批次生成工具')
    parser.add_argument('--repo-root', required=True, help='仓库根目录路径')
    parser.add_argument('--function-doc-list', required=True, help='功能文档列表（JSON 格式）')
    ...
    list_attr = getattr(args, 'function-doc-list'.replace('-', '_'))
    items = load_list(list_attr)
    print(f"找到 {len(items)} 个功能文档需要处理")
    if not items:
    ...
    print(f"批次生成完成，总共创建了 {total_batches} 个批次（共 {len(items)} 个功能文档）。")
