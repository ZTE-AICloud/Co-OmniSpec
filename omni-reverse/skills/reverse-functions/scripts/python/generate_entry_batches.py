...
"""
入口识别批次生成工具
基于输入列表（--file-list）按"批次大小 + Token预算"动态分组，生成批次文件。
供 reverse-functions 对应阶段使用。

兼容 Windows 和 Linux 系统。
使用方法:
    python generate_entry_batches.py --repo-root <repo_root> --file-list <list_json> [--batch-size <size>] [--max-tokens <tokens>]

参数:
    --repo-root:  仓库根目录路径
    --file-list:     要处理的文件列表（JSON 格式：数组，或 {"items": [...]} / {"files": [...]}）
...
产出（写入 {repo_root}/.cache/reverse/functions/）:
    ...
    - entry_identification-batch-status.json
"""
import os
import sys
import json
...
def main():
    parser = argparse.ArgumentParser(description='入口识别批次生成工具')
    parser.add_argument('--repo-root', required=True, help='仓库根目录路径')
    parser.add_argument('--file-list', required=True, help='要处理的文件列表（JSON 格式）')
    ...
    list_attr = getattr(args, 'file-list'.replace('-', '_'))
    items = load_list(list_attr)
    print(f"找到 {len(items)} 个文件需要处理")
    if not items:
    ...
    print(f"批次生成完成，总共创建了 {total_batches} 个批次（共 {len(items)} 个文件）。")
