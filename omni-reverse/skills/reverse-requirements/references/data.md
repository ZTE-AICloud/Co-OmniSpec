...
- **清单文件**：`{REPO_ROOT}/omni-doc/specs/requirements/需求清单.md`
- **变量名**：`REQUIREMENTS_INVENTORY_FILE`
- **生成时机**：全部 `{ID_PREFIX}-*.md` 独立需求文件写入完成后生成；禁止在独立文件未就绪时单独生成清单。
- **模板来源**（按优先级查找，与本 Skill 的 `templates/` 目录一致）：
  1. `{REPO_ROOT}/.claude/skills/reverse-requirements/templates/reverse-requirement-inventory-template.md`（安装到目标项目后）
  2. `{REPO_ROOT}/skills/reverse-requirements/templates/reverse-requirement-inventory-template.md`（OmniSpec 源码仓或未安装 `.claude/skills` 时）
  3. `{REPO_ROOT}/claude/skills/reverse-requirements/templates/reverse-requirement-inventory-template.md`（备用路径）
- **超链接规范**：
  - 清单与独立需求文件同目录，链接使用**相对路径**：`[显示文本](./{ID_PREFIX}-{NNN}-{需求简述}.md)`
  - 显示文本优先使用 frontmatter 的 `name`，否则使用文件名（不含 `.md`）
  - 清单自身（`需求清单.md`）不得出现在需求列表表中
- **表格列**（至少包含）：需求ID、需求名称、类型、来源场景、需求文件（超链接列）
- **重录清理**：`--clear-cache` 时与 `{ID_PREFIX}-*.md` 一并删除 `需求清单.md` 后重新生成

### 单需求文件内容格式

**模板来源**：单需求文件的格式**不在此处定义**，生成时从 **`.omni-infra/metamodel/1.requirement-template.md`** 读取并按其结构填充。
