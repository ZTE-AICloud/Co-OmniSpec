...
- **需求清单**：`{REPO_ROOT}/omni-doc/specs/requirements/需求清单.md`
  - 汇总全部独立需求；「需求文件」列必须为指向同目录下对应 `.md` 的 Markdown 超链接
  - 模板见本 Skill 的 `templates/reverse-requirement-inventory-template.md`（路径约定见 data.md）

## 文件内容格式

**不在此处写死模板**。生成每个需求文件时：

1. **读取模板**：从 **`.omni-infra/metamodel/1.requirement-template.md`** 读取格式定义（frontmatter 字段与正文结构）。
2. **按模板填充**：将当前需求的 id、name、type、description 及 EARS 主需求/子需求填入模板对应位置后写出。

若运行环境为安装后的目标项目，模板路径为 **`.omni-infra/metamodel/1.requirement-template.md`**（安装时 OmniSpec 的 `omni-infra/` 等资源落在目标仓库的 `.omni-infra/` 下）。
...
9. 生成 `需求清单.md`（含各需求超链接，见下文「生成需求清单」）
10. 创建 `{REPO_ROOT}/.cache/reverse/requirements/internal/requirement_split_success.flag`（可选）
11. 更新缓存状态；交互模式下可再次确认后结束

### 1. 解析参数与检查缓存

- 从 `$ARGUMENTS` 解析：`--clear-cache`、`--interactive`。若带 `--clear-cache` 或用户要求重录，则忽略 `requirement_split.confirmed`，执行本阶段；且在生成文件前先删除输出目录下已有 `{ID_PREFIX}-*.md` 与 `需求清单.md`，再重新生成。
...
- 对每条需求：按 `.omni-infra/metamodel/1.requirement-template.md` 格式写入 `{REPO_ROOT}/omni-doc/specs/requirements/{ID_PREFIX}-XXX-需求简述.md`（先写 YAML frontmatter，再写 EARS 正文）
- 重录时：先删除该目录下已有 `{ID_PREFIX}-*.md` 与 `需求清单.md`，再生成

### 6. 生成需求清单

- **前置条件**：步骤 5 中所有 `{ID_PREFIX}-*.md` 已写入且可通过 Glob 列举。
- **读取模板**：按 data.md 中优先级查找 `reverse-requirement-inventory-template.md`；若均不存在，使用下列最小结构：
...
|--------|----------|------|----------|----------|
| {ID} | {name} | {type} | {SCN-001, SCN-002} | [{link_text}](./{filename}.md) |
```

- **数据来源**：以本阶段已生成的独立需求文件为准（读取各文件 frontmatter 的 `id`、`name`、`type`、`description`；来源场景从 EARS 正文或 frontmatter 中的 `[来源: SCN-XXX]` / `source_scenarios` 等字段提取，多个场景用逗号分隔）。
- **超链接**（必须）：
  - 格式：`[{link_text}](./{filename}.md)`，`filename` 为实际文件名（含 `.md`）
  - `link_text` 优先 `name`，否则为去掉扩展名的文件名
  - 路径相对于 `需求清单.md` 所在目录（即 `./` 前缀）
- **统计章节**（可选）：按 `type` 汇总需求类型统计；按来源场景 ID 汇总条数。
- **写入路径**：`{REPO_ROOT}/omni-doc/specs/requirements/需求清单.md`
- **禁止**：将 `需求清单.md` 自身列入表格；链接指向不存在的文件。
...
- [ ] 编号连续无遗漏
- [ ] 需求简述简洁准确，子需求归属正确
- [ ] 已生成 `需求清单.md`，且表中「需求文件」列每条均为有效的 `./{filename}.md` 相对超链接
- [ ] 清单行数与独立需求文件数一致（不含 `需求清单.md`）
- [ ] 已创建成功标记文件（若约定需要）
- [ ] 每条 EARS 句式有来源场景 ID 标注（`[来源: SCN-XXX]` 或 frontmatter 中对应字段）
- [ ] EARS 句式类型与场景事件类型匹配；不匹配者标注为"待确认"且不作为正式需求计入完成数
