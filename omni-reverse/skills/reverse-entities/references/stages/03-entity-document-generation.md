...
- 定义实体模板路径：`{REPO_ROOT}/.omni-infra/metamodel/6.entity-template.md`
...
4.2. **加载实体模板**
   - 🔴 **强制要求**：必须加载实体文档模板
   - 强制使用默认模板：`{REPO_ROOT}/.omni-infra/metamodel/6.entity-template.md`
   - 读取模板文件内容
...
4.3. **生成实体文档（基于模板填充）**
   ...
   - 调用实体文档生成脚本：
     - Linux/macOS: `python3 {REPO_ROOT}/.omni-infra/scripts/python/reverse_entities/entity_list_generator.py --repo-root {REPO_ROOT} --input-file {consolidated_entities_file} --template-file {template_file} --output-dir {output_dir} --mode generate-documents`
     - Windows: `python {REPO_ROOT}\.omni-infra\scripts\python\reverse_entities\entity_list_generator.py --repo-root {REPO_ROOT} --input-file {consolidated_entities_file} --template-file {template_file} --output-dir {output_dir} --mode generate-documents`
   - 脚本参数说明：
     - `--repo-root`：仓库根目录
   ...
   - 🔴 **文档生成逻辑**：
     - 为每个实体生成独立的MD文件，🔴 **输出内容必须与** `{REPO_ROOT}/.omni-infra/metamodel/6.entity-template.md` **的格式完全一致**（章节标题、层级、顺序、frontmatter 键名均不得更改）
...
4.4. **生成实体清单**
   - 🔴 **强制要求**：必须生成实体清单文件
   - 调用实体清单生成脚本：
     - Linux/macOS: `python3 {REPO_ROOT}/.omni-infra/scripts/python/reverse_entities/entity_list_generator.py --repo-root {REPO_ROOT} --entity-dir {output_dir} --output {output_dir}/实体清单.md --mode generate-list`
     - Windows: `python {REPO_ROOT}\.omni-infra\scripts\python\reverse_entities\entity_list_generator.py --repo-root {REPO_ROOT} --entity-dir {output_dir} --output {output_dir}\实体清单.md --mode generate-list`
   - 脚本参数说明：
     - `--repo-root`：仓库根目录
...
- **实体模板**（必需）：
  - 强制使用：`{REPO_ROOT}/.omni-infra/metamodel/6.entity-template.md`
...
- **文档生成要求**：
  ...
  - 🔴 实体文档必须严格按 `{REPO_ROOT}/.omni-infra/metamodel/6.entity-template.md` 生成：仅包含「## 实体」「## 实体结构」「## 属性说明」「## 方法说明」「## 职责说明」五节及 frontmatter；禁止使用「基本属性」「实体基本信息」「业务属性」「关键特性」「关系属性」「使用场景」「技术实现」「风险与注意事项」等非模板章节
  - 实体序号必须连续，从001开始
