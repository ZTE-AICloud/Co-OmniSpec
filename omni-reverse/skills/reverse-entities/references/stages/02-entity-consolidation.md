...
4.3. **步骤0：规则去重（基于entity_id）**
   ...
   - 调用实体融合脚本执行规则去重：
     - Linux/macOS: `python3 {REPO_ROOT}/.omni-infra/scripts/python/reverse_entities/entity_consolidator.py --repo-root {REPO_ROOT} --input-dir {entity_extraction_dir} --output-dir {entity_consolidation_dir} --step rule-dedup`
     - Windows: `python {REPO_ROOT}\.omni-infra\scripts\python\reverse_entities\entity_consolidator.py --repo-root {REPO_ROOT} --input-dir {entity_extraction_dir} --output-dir {entity_consolidation_dir} --step rule-dedup`
   - 脚本参数说明：
     - `--repo-root`：仓库根目录
     - `--input-dir`：实体抽取输出目录
     - `--output-dir`：实体融合输出目录
...
4.4. **步骤1-N：多轮融合迭代（默认最多3轮）**
   ...
   - 调用实体融合脚本执行基本信息融合：
     - Linux/macOS: `python3 {REPO_ROOT}/.omni-infra/scripts/python/reverse_entities/entity_consolidator.py --repo-root {REPO_ROOT} --input-dir {entity_extraction_dir} --output-dir {entity_consolidation_dir} --step basic-merge --round {round_num}`
     - Windows: `python {REPO_ROOT}\.omni-infra\scripts\python\reverse_entities\entity_consolidator.py --repo-root {REPO_ROOT} --input-dir {entity_extraction_dir} --output-dir {entity_consolidation_dir} --step basic-merge --round {round_num}`
   - 🔴 **批次处理**：
   ...
   - 🔴 **强制要求**：必须为合并后的实体组整合类图
   - 调用实体融合脚本执行类图整合：
     - Linux/macOS: `python3 {REPO_ROOT}/.omni-infra/scripts/python/reverse_entities/entity_consolidator.py --repo-root {REPO_ROOT} --input-dir {entity_extraction_dir} --output-dir {entity_consolidation_dir} --step class-diagram-merge --round {round_num}`
     - Windows: `python {REPO_ROOT}\.omni-infra\scripts\python\reverse_entities\entity_consolidator.py --repo-root {REPO_ROOT} --input-dir {entity_extraction_dir} --output-dir {entity_consolidation_dir} --step class-diagram-merge --round {round_num}`
   - 🔴 **处理逻辑**：
     - 收集合并组中所有源实体的类图
     - 如果只有一个类图，直接使用
...
4.5. **步骤4：实体价值评估**
   ...
   - 调用实体融合脚本执行价值评估：
     - Linux/macOS: `python3 {REPO_ROOT}/.omni-infra/scripts/python/reverse_entities/entity_consolidator.py --repo-root {REPO_ROOT} --input-dir {entity_extraction_dir} --output-dir {entity_consolidation_dir} --step value-evaluation`
     - Windows: `python {REPO_ROOT}\.omni-infra\scripts\python\reverse_entities\entity_consolidator.py --repo-root {REPO_ROOT} --input-dir {entity_extraction_dir} --output-dir {entity_consolidation_dir} --step value-evaluation`
   - 🔴 **批次处理**：
...
4.6. **生成融合结果**
   ...
   - 调用实体融合脚本生成最终结果：
     - Linux/macOS: `python3 {REPO_ROOT}/.omni-infra/scripts/python/reverse_entities/entity_consolidator.py --repo-root {REPO_ROOT} --input-dir {entity_extraction_dir} --output-dir {entity_consolidation_dir} --step generate-result`
     - Windows: `python {REPO_ROOT}\.omni-infra\scripts\python\reverse_entities\entity_consolidator.py --repo-root {REPO_ROOT} --input-dir {entity_extraction_dir} --output-dir {entity_consolidation_dir} --step generate-result`
   - 🔴 **生成文件**：
     - **融合后的实体列表**：
