...
- **OmniSpec 项目模板**（`{REPO_ROOT}/.omni-infra/templates/`）：`interface-type-selection-template.md`、`constraint-configuration-template.md`、`final-confirmation-template.md` 等交互式配置模板
- **OmniSpec 确认模板**（`reverse-shared/references/confirmation-template.md`）：统一确认机制模板
- **本 Skill 捆绑脚本**（`${CLAUDE_SKILL_DIR}/references/scripts/`）：`estimate_interface_counts.py` 等 Python 脚本
...
- 读取接口类型选择模板：`{REPO_ROOT}/.omni-infra/templates/interface-type-selection-template.md`
...
### 6. [ ] 用户确认扫描配置 - 约束规则配置
- **🔴 前置条件**：仅在配置文件不存在时执行此步骤
- 读取约束规则配置模板：`{REPO_ROOT}/.omni-infra/templates/constraint-configuration-template.md`
- **🔴 执行统一确认机制**：按照 `reverse-shared/references/confirmation-template.md` 中的"过程中确认模板 - 类型1：配置选择确认"执行
  - 默认配置：空约束规则
...
- **🔴 前置条件**：仅在配置文件不存在时执行此步骤
- 读取最终确认模板：`{REPO_ROOT}/.omni-infra/templates/final-confirmation-template.md`
- **🔴 执行统一确认机制**：按照 `reverse-shared/references/confirmation-template.md` 中的"过程中确认模板 - 类型1：配置选择确认"执行
