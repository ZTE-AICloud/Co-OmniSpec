...
def run_reverse_identify(
    repo_root,
    input_base_dir,
    output_base_dir=None,  # type: Optional[str]
    no_llm=False,
):
    """
    ...
    # 1. 解析 reverse_syntax_parser/main.py：优先 `.omni-infra/scripts/python/`，其次 OmniSpec 源码树 `scripts/python/`
    scripts_root_candidates = [
        repo_root_path / ".omni-infra" / "scripts" / "python",
        repo_root_path / "scripts" / "python",
    ]
    main_py = None  # type: Optional[Path]
    ...
    if not main_py:
        raise FileNotFoundError(
            f"未找到 reverse_syntax_parser/main.py，请确认已安装到 `.omni-infra/scripts/python/` 或源码树存在 `scripts/python/`"
        )
