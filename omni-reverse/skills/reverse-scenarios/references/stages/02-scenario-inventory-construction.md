...
- 如果 `confirmed == true` 且 `scenario-list.json` 存在：跳过阶段2，使用缓存结果（`场景清单.md` 在阶段3 生成，不作为本阶段跳过条件）
...
4.3. **调用批次生成脚本自动分批处理（文件数量 > 20时执行）**：
   ...
   - 🔴 动态批次规划：
     - 基于用户约束规则和特征筛选获取文件列表
     - 调用批次生成脚本自动分批处理：
       - Linux/macOS: `python3 ${DSDD}/scripts/python/generate_scenario_batches.py --repo-root {REPO_ROOT} --file-list {file_list_json}`
       - Windows: `python {REPO_ROOT}\scripts\python\generate_scenario_batches.py --repo-root {REPO_ROOT} --file-list {file_list_json}`
       - 或使用平台原生脚本：
         - Linux/macOS: `${DSDD}/scripts/bash/generate-scenario-batches.sh --repo-root {REPO_ROOT} --file-list {file_list_json}`
...
4.4. **分轮启动多个子agent并发处理多个批次（分轮执行策略）**
   ...
   - 🔴 **并行处理策略**：
     ...
     - 使用新的批量获取脚本获取多个待处理批次：
       - Linux/macOS: `${DSDD}/scripts/bash/get-next-batches.sh --repo-root {REPO_ROOT} --batch-count 2`
...
4.5. **收集子agent处理结果**
   ...
   - 🔴 **批量状态更新**：
     - 使用新的批量状态更新脚本同时更新所有已完成批次的状态：
       - Linux/macOS: `${DSDD}/scripts/bash/update-batches-status.sh --repo-root {REPO_ROOT} --batch-updates '{batch_updates_json}'`
...
4.8. **检查是否还有未处理的批次【必须执行的步骤】**
   ...
   - 🔴 调用获取批次脚本检查是否还有待处理批次：
       - 使用新的批量获取脚本获取多个待处理批次：
       - Linux/macOS: `${DSDD}/scripts/bash/get-next-batches.sh --repo-root {REPO_ROOT} --batch-count 5`
   ...
   - 🔴 调用进度跟踪脚本获取当前处理进度：
     - Linux/macOS: `${DSDD}/scripts/bash/get-next-batch.sh --repo-root {REPO_ROOT} --action get-summary`
...
4.12. **扫描完成后调用脚本自动合并生成最终场景文件scenario-list.json**：
   - 调用自动合并脚本处理所有批次文件：
     - Linux/macOS: `python3 ${DSDD}/scripts/python/merge_scenario_results.py {REPO_ROOT}`
...
### 调用的子代理
- 类型：`omni-reverse:scenario-recognizer`
- 规范文档：[references/implementation/scenario-recognition.md](../implementation/scenario-recognition.md)
- 职责：识别单个文件中的场景，将结果写入临时 JSON 文件并返回极简状态

### 调用方式
使用 Agent 工具调用子代理：
```markdown
使用 Agent 工具调用 scenario-recognizer 子代理：
- subagent_type: omni-reverse:scenario-recognizer
- description: "场景清单批次扫描"
- prompt: |
  请处理批次文件 {batch_file_path} 中的所有文件。
  输入：场景模式特征(scenario-patterns.json)、约束规则(constraints.json)、Few-shot示例(few-shot-examples.json)
  输出：生成 {output_file}，包含该批次识别出的所有场景
  规范：参考 scenario-recognition.md 执行
```

### 批次处理规范
- **分批阈值**：文件数 > 20 时自动分批
- **并发数量**：每轮最多2个子代理
- **批次大小**：每批包含多个文件，由批次生成脚本自动分配
- **处理流程**：启动子代理 → 等待完成 → 验证状态 → 收集结果 → 压缩上下文

### 返回值处理
1. 子代理返回极简状态（成功/失败）和输出文件路径
2. 主代理读取输出文件（`scenario-list-batch-{batch_number}.json`）
3. 验证 JSON 格式完整性
4. 合并所有批次结果生成 `scenario-list.json`

### 错误处理
- **重试机制**：调用失败时最多重试2次，间隔递增（10s、30s）
- **降级方案**：重试失败后将批次状态标记为 "failed"，继续处理其他批次
- **状态记录**：失败信息记录到批次状态文件

### 超时与并发控制
- **超时时间**：子代理调用超时5分钟
- **超时处理**：超时后终止任务，标记为 "failed"
- **并发策略**：每轮2个子代理，完成后执行 /compact 再处理下一轮
