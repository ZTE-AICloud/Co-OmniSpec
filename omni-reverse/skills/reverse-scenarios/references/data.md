...
omni-scenarios.sh \
  ...
  --template-inventory "$REPO_ROOT/skills/reverse-scenarios/templates/reverse-scenario-inventory-template.md" \
  --template-scenario "$REPO_ROOT/.omni-infra/templates/default/reverse-scenario-detail-template.md" \
...
omni-scenarios.ps1 `
  ...
  -TemplateInventory "$RepoRoot\skills\reverse-scenarios\templates\reverse-scenario-inventory-template.md" `
  -TemplateScenario "$RepoRoot\.omni-infra\templates\default\reverse-scenario-detail-template.md" `
  -OutputDir "$OutputDir" `
  -NonInteractive
```
...
- 文稿类输出文件：
  - 场景清单文档：`{SCENARIO_OUTPUT_DIR}/场景清单.md`（变量名 `SCENARIO_INVENTORY_FILE`）
  - 单场景文档：`{SCENARIO_OUTPUT_DIR}/SCN-XXX-*.md`

### 场景清单文档（阶段3 汇总输出）

- **清单文件**：`{REPO_ROOT}/omni-doc/specs/scenarios/场景清单.md`
- **生成时机**：阶段3 全部 `SCN-XXX-*.md` 写入完成后生成；禁止在单场景文档未就绪时单独生成清单。
- **数据来源**：读取 `{SCENARIO_CACHE_DIR}/scenario-list.json` 的 `scenarios` 数组，并与输出目录下实际 `SCN-*.md` 文件名对齐。
- **模板来源**（按优先级查找）：
  1. `{REPO_ROOT}/.claude/skills/reverse-scenarios/templates/reverse-scenario-inventory-template.md`（安装到目标项目后）
  2. `{REPO_ROOT}/skills/reverse-scenarios/templates/reverse-scenario-inventory-template.md`（OmniSpec 源码仓）
  3. `{REPO_ROOT}/claude/skills/reverse-scenarios/templates/reverse-scenario-inventory-template.md`（备用路径）
- **超链接规范**：
  - 同目录相对路径：`[显示文本](./SCN-XXX-场景名称.md)`
  - `scenario_doc_file` 取自 `scenario-list.json` 中的 `doc_file`/`filename` 字段，或与 `scenario_id`+`scenario_name` 推导的文件名；须与磁盘文件一致
  - 清单自身（`场景清单.md`）不得出现在场景列表表中
- **重录清理**：`--clear-cache` 时与 `SCN-*.md` 一并删除 `场景清单.md` 后重新生成
- 脚本应在 stdout 中输出：
  - 实际生成的文件数量
  - 输出目录路径
  - 可能的告警或错误提示
