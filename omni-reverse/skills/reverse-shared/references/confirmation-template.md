...
> **路径变量约定**：本模板中 `${DSDD}` 指向共享插件 omni-dsdd 的安装根，由 `omni-reverse/scripts/resolve-dsdd-root.sh` 解析得到。执行前先取值：
...
2. **调用统一确认机制**（按以下流程执行）：
   - 调用脚本获取交互模式状态：
     - Linux: `bash ${DSDD}/scripts/bash/check-interactive-mode.sh "$ARGUMENTS"`
     - Windows: `pwsh ${DSDD}/scripts/powershell/check-interactive-mode.ps1 "$ARGUMENTS"`
...
- **Linux/macOS**：`bash ${DSDD}/scripts/bash/check-interactive-mode.sh "$ARGUMENTS"`
- **Windows**：`pwsh ${DSDD}/scripts/powershell/check-interactive-mode.ps1 "$ARGUMENTS"`
