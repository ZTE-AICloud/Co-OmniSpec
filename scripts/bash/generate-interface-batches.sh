...
使用方法:
    ./generate-interface-batches.sh --repo-root <repo_root> --file-list <file_list_json> [--batch-size <size>] [--max-tokens <tokens>] [--force]
...
cleanup_legacy_interface_batch_mapping() {
    local legacy_file="$CACHE_DIR/interface-batch-mapping.json"
    [[ ! -f "$legacy_file" ]] && return 0
    command -v jq >/dev/null 2>&1 || return 0

    # 若所有 batch_file 都是 batch-details-*.json，则删除该遗留文件
    local is_legacy
    is_legacy=$(jq -r '
        (.batches // []) as $bs |
        ([$bs[]? | .batch_file // ""] | all(startswith("batch-details-"))) // false
    ' "$legacy_file" 2>/dev/null || echo "false")

    if [[ "$is_legacy" == "true" ]]; then
        rm -f "$legacy_file" 2>/dev/null || true
        echo "已清理遗留的 interface-batch-mapping.json（扫描阶段重复映射）" >&2
    fi
}
