...
class ClassParser:
    ...
    def parse_class(self, class_content: str, filename: str) -> Tuple[str, Dict[str, Any]]:
        ...
        class_pattern = r'''
            ...
            (\w+)                                       # 类名
            (?:<([^>]+)>)?                             # 可选泛型参数
            (?:\s+extends\s+(\w+(?:<[^>]*>)?(?:\.\w+)*))? # 可选父类
            (?:\s+implements\s+([^{]+?))?              # 可选接口实现
            \s*\{                                      # 类体开始
        '''
        
        match = re.search(class_pattern, class_content, re.VERBOSE | re.MULTILINE)
        ...
        extends_class = match.group(6)
        implements_interfaces = match.group(7)
        ...
        class_info = {
            ...
            "methods": self._parse_methods(class_body, class_name, filename, unique_id),
    ...
    def parse_interface(self, interface_content: str, filename: str) -> Tuple[str, Dict[str, Any]]:
        ...
        match = re.search(interface_pattern, interface_content, re.VERBOSE | re.MULTILINE)
        ...
        generic_params = match.group(5)
        extends_interfaces = match.group(6)
    ...
    def _parse_methods(self, class_body: str, class_name: str, filename: str, class_uuid: str = None) -> Dict[str, Dict[str, Any]]:
        """解析类方法"""
        methods = self.method_parser.parse_all_methods_from_class(class_body, class_name, filename)
