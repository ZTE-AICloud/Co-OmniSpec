...
def main():
    ...
    if not project_name:
        # 尝试从配置文件读取
        config_file = os.path.join(repo_root, '.infra', 'config.yaml')
    ...
    if project_name:
        project_template = os.path.join(repo_root, '.infra', 'templates', project_name, 'reverse-interface-inventory-template.md')
        if os.path.exists(project_template):
            template_file = project_template
    
    # 2. 尝试项目根目录下的默认模板（向后兼容）
    if not template_file:
        project_default = os.path.join(repo_root, '.infra', 'templates', 'reverse-interface-inventory-template.md')
        if os.path.exists(project_default):
            template_file = project_default
    
    # 3. 尝试项目根目录下的 default 子目录模板
    if not template_file:
        project_default_dir = os.path.join(repo_root, '.infra', 'templates', 'default', 'reverse-interface-inventory-template.md')
    ...
    if os.path.exists(validator_script):
        proc = subprocess.run([sys.executable, validator_script, repo_root], capture_output=True, text=True)
