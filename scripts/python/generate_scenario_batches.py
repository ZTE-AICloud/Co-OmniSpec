...
兼容Windows和Linux系统
使用方法:
    python generate_scenario_batches.py --repo-root <repo_root> --file-list <file_list_json> [--batch-size <size>] [--max-tokens <tokens>]
...
def initialize_batch_status(cache_dir: str, total_items: int, total_batches: int) -> None:
    """初始化批次状态文件"""
    batch_status_file = os.path.join(cache_dir, 'scenario_scanning-batch-status.json')
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='场景扫描批次生成工具')
