...
{{/description}}
"@
    $detailTemplate | Set-Content -Path (Join-Path -Path $testDir -ChildPath ".infra/templates/reverse-interface-detail-template.md") -Encoding UTF8
...
{{/interfaces}}
"@
    $inventoryTemplate | Set-Content -Path (Join-Path -Path $testDir -ChildPath ".infra/templates/reverse-interface-inventory-template.md") -Encoding UTF8
