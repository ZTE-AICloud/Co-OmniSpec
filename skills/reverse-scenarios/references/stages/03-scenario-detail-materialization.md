...
4.1. **获取已处理场景信息**
   ...
   - 🔴 调用进度跟踪脚本获取当前处理进度：
     - Linux/macOS: `{REPO_ROOT}/scripts/bash/reverse/scenarios/utils/get_scenario_detail_progress.sh {REPO_ROOT}`
     - Windows: `powershell -ExecutionPolicy Bypass -File {REPO_ROOT}\scripts\powershell\reverse\scenarios\utils\Get-ScenarioDetailProgress.ps1 -RepoRoot {REPO_ROOT}`
   - 🔴 分析场景清单文件，确定当前处理状态：
...
4.2. **创建场景批次文件**
   ...
   - 🔴 **批次文件创建**：仅在批次映射文件不存在或无效时才调用批次生成脚本：
     - Linux/macOS: `{REPO_ROOT}/scripts/bash/reverse/scenarios/utils/create_scenario_detail_batches.sh --repo-root {REPO_ROOT}`
     - Windows: `powershell -ExecutionPolicy Bypass -File {REPO_ROOT}\scripts\powershell\reverse\scenarios\utils\Create-ScenarioDetailBatches.ps1 -RepoRoot {REPO_ROOT}`
...
4.3. **获取下一个要执行的场景批次**
   ...
   - 🔴 调用获取批次脚本获取待处理批次（最多3个）：
     - Linux/macOS: `{REPO_ROOT}/scripts/bash/reverse/scenarios/utils/get_next_scenario_detail_batches.sh --repo-root {REPO_ROOT} --batch-count 3`
     - Windows: `powershell -ExecutionPolicy Bypass -File {REPO_ROOT}\scripts\powershell\reverse\scenarios\utils\Get-NextScenarioDetailBatches.ps1 -RepoRoot {REPO_ROOT} -BatchCount 3`
   - 🔴 **处理不同场景**：
...
4.4. **更新批次状态为processing（启动前）**
   ...
   - 🔴 调用批量状态更新脚本将批次状态标记为"processing"：
     - Linux/macOS: `{REPO_ROOT}/scripts/bash/reverse/scenarios/utils/update_scenario_detail_batches_status.sh --repo-root {REPO_ROOT} --batch-updates '{batch_updates_json}'`
     - Windows: `powershell -ExecutionPolicy Bypass -File {REPO_ROOT}\scripts\powershell\reverse\scenarios\utils\Update-ScenarioDetailBatchesStatus.ps1 -RepoRoot {REPO_ROOT} -BatchUpdates "{batch_updates_json}"`
