...
4.3. **调用批次生成脚本自动分批处理（场景数量 > 20时执行）**：
   ...
   - 🔴 动态批次规划：
     - 基于场景信息获取场景列表
     - 调用批次生成脚本自动分批处理：
       - Linux/macOS: `python3 {REPO_ROOT}/scripts/python/generate_function_batches.py --repo-root {REPO_ROOT} --scenario-list {scenario_list_json}`
       - Windows: `python {REPO_ROOT}\scripts\python\generate_function_batches.py --repo-root {REPO_ROOT} --scenario-list {scenario_list_json}`
     - 生成批次映射和详细文件：
...
4.4. **分轮启动多个子agent并发处理多个批次（分轮执行策略）**
   ...
   - 🔴 **并行处理策略**：
     ...
     - 使用批量获取脚本获取多个待处理批次：
       - Linux/macOS: `{REPO_ROOT}/scripts/bash/get-next-batches.sh --repo-root {REPO_ROOT} --batch-count 2 --stage-type functions --stage-name function_partitioning`
