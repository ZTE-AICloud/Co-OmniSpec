...
- 严格按阶段顺序：P0 → P1 → P2 → P3 → P4
- 同阶段内可并行时，可一次处理多份 03-rule-*.md（建议每批不超过 5 个以控制 Token）

#### 单规则执行步骤

1. 确定当前规则 ID（如 `00-architecture`、`05-logging`）。
2. **按类型加载模板（推荐）**：优先读取 `{REPO_ROOT}/.infra/templates/default/reverse-rules-templates/{规则ID}.template.md`；若存在，则根据 `01-features.md` 与代码库实际情况填充模板中的占位符（如 `{{MAIN_LANGUAGE}}`、`{{SCOPE}}`、`{{PRINCIPLES}}`、`{{SPECIFICATIONS}}` 等，详见 data.md），直接得到该规则的 .mdc 内容；若不存在，继续下一步。
...
- **每个类型一个模板**：每个规则类型对应一个模板文件，位于 `{REPO_ROOT}/.infra/templates/default/reverse-rules-templates/{规则ID}.template.md`，模板内使用占位符（如 `{{RULE_ID}}`、`{{MAIN_LANGUAGE}}`、`{{SCOPE}}`、`{{PRINCIPLES}}`、`{{SPECIFICATIONS}}`、`{{IMPLEMENTATION_GUIDE}}`、`{{CHECKLIST}}` 等），由 Agent 根据代码库实际情况填充后得到最终 .mdc。
