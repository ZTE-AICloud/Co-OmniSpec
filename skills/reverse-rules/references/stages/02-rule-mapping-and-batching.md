...
- **指定模版填充**：若用户传入 `--template <file>` 或存在项目模板 `{REPO_ROOT}/.infra/templates/reverse-rule-execution-template.md`，则优先读取该模板，将占位符（如 `{RULE_ID}`、`{RULE_NAME}`、`{RULE_DESCRIPTION}`、`{GLOBS_PATTERN}`、`{ALWAYS_APPLY}` 等）替换为当前规则信息后，为每条规则生成对应的 03-rule-*.md；否则使用下述内置结构。
