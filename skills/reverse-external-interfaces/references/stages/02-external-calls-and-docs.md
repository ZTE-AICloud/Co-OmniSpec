...
- **单文件内容格式**（推荐做法，模板外置）：
  1. **读取模板**：在生成每个 `EXTERNAL-API_{序号}_{简短描述}.md` 前，从仓库根相对路径读取 **`.infra/metamodel/9.external-interface-template.md`**，作为唯一格式规范来源（不在本阶段文件中内嵌模板内容）。
...
- 单接口文档的内容格式以 **`.infra/metamodel/9.external-interface-template.md`** 为准，生成时从该模板读取并填充，不在此阶段文件中内嵌模板。
- 所有与用户交互使用中文。
