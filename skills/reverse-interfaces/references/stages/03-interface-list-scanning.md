...
- **执行统一确认机制**：按照 `reverse-shared/references/confirmation-template.md` 中的"过程中确认模板 - 类型1：配置选择确认"执行
  ...
  - **🔴 方式判定优先级**（避免“已设默认 A 仍执行 B”）：
    1. 若存在 `{REPO_ROOT}/.cache/user_input/interface-scan-mode.json` 且其 `mode` 字段为 `"B"` → 使用方式 B（用户显式预选 B）。
  ...
  - **可选预配置**：若用户希望预选方式 B 且无需交互，可在 `{REPO_ROOT}/.cache/user_input/` 下创建 `interface-scan-mode.json` 并设置 `"mode": "B"`（格式参考 `.cache/user_input/interface-scan-mode_example.json`）；否则本步检查该文件是否存在且 mode 为 B，否则按默认 A 或交互选择。
...
- 0. **默认使用Python3.6以上版本并且带齐脚本参数**默认使用python3 执行；带齐脚本参数 --repo-root {REPO_ROOT} --codebase {REPO_ROOT} --output-dir {input_base_dir}
  ...
  6. **若脚本执行失败，或执行后仍未生成出上述三个文件**：**退回使用方式 A**，并必须：
   ...
   - 🔴 **批次文件创建（仅在确实不存在有效批次时执行）**：
     ...
     - 🔴 **脚本调用方式**：
       - Linux/macOS/Windows（推荐Python版本，跨平台通用）：
         ```bash
         python3 {REPO_ROOT}/scripts/python/generate_interface_batches.py \
           --repo-root {REPO_ROOT} \
...
3.4. **分轮启动多个子agent并发处理多个批次（分轮执行策略）**
   - 根据 `skill-instruction.md`，每轮最多启动 2 个 `interface-recognizer` 子 Agent，处理 1–2 个批次
   - 使用批量获取脚本（如 `get-next-batches`）一次获取本轮要处理的批次列表
   - 🔴 **脚本调用方式**：
     - Linux/macOS：
       ```bash
       {REPO_ROOT}/scripts/bash/get-next-batches.sh \
         --repo-root {REPO_ROOT} \
         --batch-count 2
       ```
...
3.12. **扫描完成后调用脚本自动合并生成最终接口文件interface-list.json**：
   - 使用统一的合并脚本（跨平台，仅依赖 Python）对所有批次结果进行合并：
     ```bash
     python3 {REPO_ROOT}/scripts/python/merge_interface_results.py {REPO_ROOT}
     ```
...
3B.1. **确定路径与依赖**（按顺序执行,使用python3执行，脚本需要带齐参数 --repo-root {REPO_ROOT} --codebase {REPO_ROOT} --output-dir {input_base_dir}）
   - **步骤 1：找到前置依赖生成脚本**（🔴 此步仅检查文件是否存在，不执行脚本）
     ...
     - 若两者均不存在：直接退回方式 A（须更新缓存为 mode A 并向用户输出“已退回方式 A”及原因，同 2.5 步骤 6）
   ...
   - **步骤 6：脚本执行后再次校验**
     - 若脚本执行失败（非零退出码）：退回方式 A（须更新缓存为 mode A 并输出“已退回方式 A”原因，同上 2.5 步骤 6）。
     - 若脚本执行成功，但三个文件仍未全部生成：同上，退回方式 A 并更新缓存与提示。
     - 若三个文件均已生成：继续执行 3B.2
