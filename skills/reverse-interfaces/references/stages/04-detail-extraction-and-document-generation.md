...
4.1. **获取已处理接口信息**
   ...
   - 🔴 调用进度跟踪脚本获取当前处理进度：
     - Linux/macOS: `{REPO_ROOT}/scripts/bash/reverse/interfaces/utils/get_processing_progress.sh {REPO_ROOT}`
...
4.2. **创建接口批次文件**
   ...
   - 🔴 **批次文件创建**：仅在批次映射文件不存在或无效时才调用批次生成脚本：
     - Linux/macOS: `{REPO_ROOT}/scripts/bash/reverse/interfaces/utils/create_interface_batches.sh --repo-root {REPO_ROOT}`
   ...
   - 🔴 **上下文超限回退策略（新增）**：
     ...
     - 重建命令示例（Linux/macOS）：
       ...
       {REPO_ROOT}/scripts/bash/reverse/interfaces/utils/create_interface_batches.sh --repo-root {REPO_ROOT} --force
       ```
...
4.3. **获取下一个要执行的接口批次**
   ...
   - 🔴 调用获取批次脚本获取待处理批次（最多3个）：
     - Linux/macOS: `{REPO_ROOT}/scripts/bash/reverse/interfaces/utils/get_next_interface_batches.sh --repo-root {REPO_ROOT} --batch-count 3`
...
4.4. **更新批次状态为processing（启动前）**
   ...
   - 🔴 调用批量状态更新脚本将批次状态标记为"processing"：
     - Linux/macOS: `{REPO_ROOT}/scripts/bash/reverse/interfaces/utils/update_interface_batches_status.sh --repo-root {REPO_ROOT} --batch-updates '{batch_updates_json}'`
...
4.4.5. **等待子agent完成（强制轮询检查）**
   ...
   - 🔴 **轮询检查流程**：
     1. 使用批次状态验证脚本验证批次是否真正完成：
        - Linux/macOS: `{REPO_ROOT}/scripts/bash/verify-batches-completion.sh --repo-root {REPO_ROOT} --batch-numbers '{batch_numbers_json}' --stage-type interfaces`
...
- **🔴 生成汇总接口清单**：
  - 调用接口清单生成脚本创建汇总文档：
    - Linux/macOS: `{REPO_ROOT}/scripts/bash/reverse/interfaces/utils/generate_interface_inventory.sh --repo-root {REPO_ROOT}`
