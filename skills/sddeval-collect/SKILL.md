...
1. 从当前 SDD 分支的 `changes/<branch>/` 目录结构中自动识别当前分支
2. 扫描目标目录（如 `networking_zte`）下的所有代码变更
...
输出文件路径: `changes/<branch>/evalset/configiEval.generated.json`

**JSON 格式**:
```json
{
    "api": {
        "url": "https://maas-apigateway.dt.zte.com.cn/model/qwen3-coder-480b-a35b-instruct-fp8/v1/chat/completions",
        "key": "j4jrhmbjepo1t9rxjk77fmv93w547rq5",
...
- **成功**: 生成 `changes/<branch>/evalset/configiEval.generated.json` 并显示统计信息
- **失败**: 显示错误原因
...
4. 输出: `changes/001-TCF-5064840-vpn-service/evalset/configiEval.generated.json`
...
4. 输出: `changes/<branch>/evalset/configiEval.generated.json`
